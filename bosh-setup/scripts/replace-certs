...
$(cert_variable hm9000_ca_cert     certs/hm9000-certs/server_ca.crt)
...
certstrap_generate_certs() {
  ...
  if [ -n "$agent_cn" ]; then
    ...
    certstrap --depot-path ${depot_path} sign $agent_cn --CA server-ca
    mv -f ${depot_path}/$agent_cn.key ${depot_path}/agent.key
    mv -f ${depot_path}/$agent_cn.csr ${depot_path}/agent.csr
    mv -f ${depot_path}/$agent_cn.crt ${depot_path}/agent.crt
  fi
}
...
mkdir -p certs
pushd certs
  certstrap_generate_certs "consul-certs" "server.dc1.cf.internal" "consul_agent"
  certstrap_generate_certs "hm9000-certs" "listener-hm9000.service.cf.internal" "hm9000_client"
  certstrap_generate_certs "diego-certs" "bbs.service.cf.internal" "bbs_client"
  certstrap_generate_certs "uaa-certs" "uaa.service.cf.internal"
  certstrap_generate_certs "blobstore-certs" "blobstore.service.cf.internal"
